library ALibraryCoreSoundMusic
	...
	*/
	...
	function SetMapMusicIndexedForForce takes force whichForce, string musicList, integer index returns nothing
		if (IsPlayerInForce(GetLocalPlayer())) then
			call SetMapMusicIndexedBJ(musicList, index)
		endif
	endfunction
	...
	function PlayMusicExForPlayer takes player whichPlayer, integer fromMilliseconds, integer fadeInMilliseconds returns nothing
		if (whichPlayer == GetLocalPlayer()) then
			call PlayMusicEx(musicName, fromMilliseconds, fadeInMilliseconds)
		endif
	endfunction

	function PlayMusicExForForce takes force whichForce, boolean fadeOut returns nothing
		if (IsPlayerInForce(GetLocalPlayer(), whichForce)) then
			call PlayMusicEx(musicName, fromMilliseconds, fadeInMilliseconds)
		endif
	endfunction
	...
	function StopMusicForForce takes player whichForce, boolean fadeOut returns nothing
		if (IsPlayerInForce(GetLocalPlayer(), whichForce)) then
			call StopMusic(fadeOut)
		endif
	endfunction
	...
	function EndThematicMusicForForce takes player whichForce returns nothing
		if (IsPlayerInForce(GetLocalPlayer(), whichForce)) then
			call EndThematicMusic()
		endif
	endfunction
