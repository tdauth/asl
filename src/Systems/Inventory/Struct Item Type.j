library AStructSystemsInventoryItemType requires optional ALibraryCoreDebugMisc, AStructCoreGeneralHashTable, AStructCoreGeneralVector, ALibraryCoreMathsConversion, ALibraryCoreInterfaceMisc
	 ...
	 * You can check some usual item type requirements such as unit strength, level or class.
	 * Besides you can specify all item type abilities to get real inventory support since
	 * permanent abilities have to be added to the unit when rucksack/equipment is closed.
	 ...
	 * \note Requirement is only checked by \ref AUnitInventory when item is equipped!
	...
	struct AItemType
		 ...
		 */
		public static method create takes integer itemType, integer equipmentType, integer requiredLevel, integer requiredStrength, integer requiredAgility, integer requiredIntelligence returns thistype
		...
		public static method createSimple takes integer itemType, integer equipmentType returns thistype
			return thistype.create(itemType, equipmentType, 0, 0, 0, 0)
		endmethod

		/// Custom item types should never be destroyed since they're heavily used by systems like AUnitInventory at game runtime (equipment).
