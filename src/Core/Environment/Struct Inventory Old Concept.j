library AStructSystemsCharacterInventory requires AStructCoreGeneralHashTable, ALibraryCoreGeneralUnit, AStructCoreStringFormat, AStructSystemsCharacterAbstractCharacterSystem, AStructSystemsCharacterCharacter, AStructSystemsCharacterItemType
	...
	*/
	...
	private struct AEquipmentData extends AItemData
		private AEquipmentType m_type
	...
	*/
	struct AInventory extends AAbstractCharacterSystem
		...
		// members
		private AIntegerVector m_equipmentItemData // container of AEquipmentData
		private AIntegerVector m_rucksackItemData // container of AItemData
		...
		public method equipmentSize takes nothing returns integer
			return this.m_equipmentData.size()
		endmethod

		public method rucksackSize takes nothing returns integer
			return this.m_rucksackItemData.size()
		endmethod

		/**
		* If size is less than current size system tries to move items on previous indices.
		*/
		public method resizeEquipment takes integer size returns nothing
		endmethod

		public method resizeRucksack takes integer size returns nothing
		endmethod

		private static method indexPage takes integer index returns integer
			return index / thistype.maxItemsPerPage
		endmethod

		private static method indexSlot takes integer index returns integer
			return index - (this.indexPage(index) * thistype.maxItemsPerPage)
		endmethod

		private method addItem takes integer itemTypeId, integer slot returns item
			local item result
			call DisableTrigger(this.m_pickupTrigger)
			set result = UnitAddItemToSlotById(this.character().unit(), itemTypeId, slot)
		...
		private method removeItem takes integer slot returns nothing
			...
			call RemoveItem(UnitItemInSlot(this.character().unit(), slot)
			call EnableTrigger(this.m_dropTrigger)
		endmethod

		private method item takes integer slot returns item
			return UnitItemInSlot(this.character().unit(), slot)
		endmethod

		private method refreshItem takes integer index, AItemData itemData returns nothing
			local integer slot
			local item whichItem
			if (this.m_page != this.indexPage(index)) then
				return
			endif

			set slot = thistype.indexSlot(index)

			if (itemData.charges() == 0) then
				call this.removeItem(slot)
			else
				if (itemData.itemTypeId() != GetItemTypeId(this.item(slot))) then
					call this.removeItem(slot)
					set whichItem = this.addItem(itemData.itemTypeId(), slot)
				else
					set whichItem = this.item(slot)
				endif
				call itemData.assignToItem(whichItem)
			endif
		endmethod

		private method equipmentData takes integer index returns AEquipmentData
			debug if (index >= this.equipmentSize() or index < 0) then
				debug call this.print("Wrong equipment item index: " + I2S(index) + ".")
				debug return 0
			debug endif
			return this.m_equipmentData[index]
		endmethod

		private method refreshEquipmentData takes integer index returns nothing
			if (not this.m_isRucksackEnabled) then
				call this.refreshItem(index, this.equipmentData(index))
			endif
		endmethod

		private method equipmentIndex takes integer itemTypeId returns integer
			local integer i = 0
			loop
				exitwhen (i == this.equipmentSize())
				if (this.equipmentData(index) != 0 and this.equipmentData(index).itemType() == itemTypeId) then
					return i
				endif
				set i = i + 1
			endloop
		...
		private method equipmentIndexFree takes nothing returns integer
			local integer i = 0
			loop
				exitwhen (i == this.rucksackSize())
				if (this.rucksackData(index) == 0) then
					return i
				endif
				set i = i + 1
			endloop
		...
		public method setEquipmentCharges takes integer index, integer charges returns boolean
			if (this.equipmentData(index) == 0 or charges < 0 or this.equipmentData(index).type().maxCharges() < charges) then
				return false
			endif
			call this.equipmentData(index).setCharges(charges)
			call this.refreshEquipmentData(index)
			if (charges == 0) then
				call this.equipmentData(index).destroy()
				set this.m_equipmentData[index] = 0
			endif
			return true
		endmethod

		private method rucksackData takes integer index returns AInventoryItemData
			debug if (index >= this.rucksackSize() or index < 0) then
				debug call this.print("Wrong rucksack index: " + I2S(index) + ".")
				debug return 0
			debug endif
			return this.m_rucksackData[index]
		endmethod

		private method refreshRucksackData takes integer index returns nothing
			if (this.m_isRucksackEnabled) then
				call this.refreshItem(index, this.rucksackData(index))
			endif
		endmethod

		private method rucksackIndex takes integer itemTypeId returns integer
			local integer i = 0
			loop
				exitwhen (i == this.rucksackSize())
				if (this.rucksackData(index) != 0 and this.rucksackData(index).itemType() == itemTypeId) then
					return i
				endif
				set i = i + 1
			endloop
		...
		private method rucksackIndexFree takes nothing returns integer
			local integer i = 0
			loop
				exitwhen (i == this.rucksackSize())
				if (this.rucksackData(index) == 0) then
		...
		public method setRucksackCharges takes integer index, integer charges returns boolean
			if (this.rucksackData(index) == 0 or charges < 0) then
				return false
			endif
			call this.rucksackData(index).setCharges(charges)
			call this.refreshRuckackData(index)
			if (charges == 0) then
				call this.rucksackData(index).destroy()
				set this.m_rucksackData[index] = 0
			endif
			return true
		endmethod
		...
		public method totalEquipmentItems takes nothing returns integer
			...
			loop
				exitwhen (i == this.equipmentSize())
				if (this.equipmentData(i) != 0) then
					set result = result + 1
				endif
				set i = i + 1
			endloop
		...
		public method totalEquipmentCharges nothing returns integer
			local integer result = 0
			local integer i = 0
			loop
				exitwhen (i == this.equipmentSize())
				if (this.equipmentData(i) != 0) then
					set result = result + this.equipmentData(i).charges()
				endif
				set i = i + 1
			endloop
			return result
		...
		public method totalRucksackCharges takes nothing returns integer
			local integer result = 0
			local integer i = 0
			loop
				exitwhen (i == this.rucksackSize())
				if (this.rucksackData(i) != 0) then
					set result = result + this.rucksackData(i).charges()
				endif
				set i = i + 1
			endloop
			return result
		...
		public method equipmentCharges takes integer itemTypeId returns integer
			local integer result = 0
			local integer i = 0
			loop
				exitwhen (i == this.equipmentSize())
				if (this.equipmentData(i) != 0 and this.equipmentData(i).itemTypeId() == itemTypeId) then
					set result = result + 1
				endif
				set i = i + 1
			endloop
		...
		public method rucksackCharges takes integer itemTypeId returns integer
			local integer result = 0
			local integer i = 0
			loop
				exitwhen (i == this.rucksackSize())
				if (this.rucksackData(i) != 0 and this.equipmentData(i).itemTypeId() == itemTypeId) then
					set result = result + 1
				endif
				set i = i + 1
			endloop
		...
		public method charges takes integer itemTypeId returns integer
			return this.rucksackCharges(itemTypeId) + this.equipmentCharges(itemTypeId)
		endmethod

		public method has takes integer itemTypeId, integer charges returns boolean
			return this.charges(itemTypeId) >= charges
		endmethod

		public method hasEquipped takes integer itemTypeId, integer charges returns boolean
			return this.equipmentCharges(itemTypeId) >= charges
		endmethod

		public method hasInRucksack takes integer itemTypeId, integer charges returns boolean
			return this.rucksackCharges(itemTypeId) >= charges

		public method remove takes integer itemTypeId, integer charges returns boolean
			local integer tmpCharges
			local integer i = 0
			loop
				exitwhen (i == this.equipmentSize())
				if (this.equipmentData(i) != 0 and this.equipmentData(i).itemTypeId() == itemTypeId) then
					set tmpCharges = IMinBJ(this.equipmentData(i).charges(), charges)
					call this.setEquipmentCharges(i, tmpCharges)
					set charges = charges - tmpCharges
					if (charges == 0) then
						return true
					endif
				endif
			...
			loop
				exitwhen (i == this.rucksackSize())
				if (this.rucksackData(i) != 0 and this.rucksackData(i).itemTypeId() == itemTypeId) then
					set tmpCharges = IMinBJ(this.rucksackData(i).charges(), charges)
					call this.setRucksackCharges(i, tmpCharges)
					set charges = charges - tmpCharges
					if (charges == 0) then
						return true
					endif
		...
		public method removeEquipment takes integer itemTypeId, integer charges returns boolean
			local integer tmpCharges
			local integer i = 0
			loop
				exitwhen (i == this.equipmentSize())
		...
		public method removeFromRucksack integer itemTypeId, integer charges returns boolean
			local integer tmpCharges
			local integer i = 0
			loop
				exitwhen (i == this.rucksackSize())
		...
		public method addToEquipment takes integer itemTypeId, integer charges returns boolean

		endmethod

		public method addToRucksack takes integer itemTypeId, integer charges returns boolean

		endmethod

		public method add takes integer itemTypeId, integer charges takes boolean
			if (AItemType.itemTypeIdHasItemType(itemTypeId)) then
				return this.addToEquipment(itemTypeId, charges)
			else
				return this.addToRucksack(itemTypeId, charges)
			endif
		endmethod
